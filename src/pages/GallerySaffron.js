...
const GalleryItem = styled(motion.div)`
  display: flex;
  background-color: rgba(50, 30, 10, 0.6); /* Brownish overlay that works with saffron */
  border-radius: 15px;
  overflow: hidden;
...
const GalleryImage = styled.div`
  ...
  &::after {
    ...
    right: 0;
    bottom: 0;
    background: linear-gradient(to right, rgba(50, 30, 10, 0.8), transparent);

    ${GalleryItem}:nth-child(even) & {
      background: linear-gradient(to left, rgba(50, 30, 10, 0.8), transparent);
    }

    @media (max-width: 992px) {
      background: linear-gradient(to top, rgba(50, 30, 10, 0.9), transparent);
    }
  }
`;
