...
const PageContainer = styled.div`
  background-color: #f5f0e8; /* Light sandy beige background */
  color: #3a2a1a; /* Darker text color for contrast */
  min-height: 100vh;
`;
...
const PageTitle = styled(motion.h1)`
  color: #b27c36; /* Slightly darker gold for better contrast */
  font-size: 2.5rem;
  margin-bottom: 20px;
  text-align: center;
...
const GalleryItem = styled(motion.div)`
  display: flex;
  background-color: rgba(255, 250, 240, 0.9); /* Very light cream background */
  border-radius: 15px;
  overflow: hidden;
...
const GalleryImage = styled.div`
  ...
  &::after {
    ...
    right: 0;
    bottom: 0;
    background: linear-gradient(to right, rgba(255, 250, 240, 0.8), transparent);

    ${GalleryItem}:nth-child(even) & {
      background: linear-gradient(to left, rgba(255, 250, 240, 0.8), transparent);
    }

    @media (max-width: 992px) {
      background: linear-gradient(to top, rgba(255, 250, 240, 0.9), transparent);
    }
  }
`;
...
const GalleryLight = () => {
  ...
  return (
    <PageContainer>
      ...
      <ContentContainer>
        <BackLink to="/freedom">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="#b27c36">
        ...
        <GalleryIntro>{galleryDescriptions.galleryIntro[language]}</GalleryIntro>
